...
status_t Visualizer::setCaptureSize(uint32_t size)
{
    ...
    memcpy((char*)&p->data+sizeof(int32_t), &size, sizeof(size));
...
status_t Visualizer::setScalingMode(uint32_t mode) {
    ...
    memcpy((char*)&p->data+sizeof(int32_t), &mode, sizeof(mode));
    status_t status = setParameter(p);
...
status_t Visualizer::setMeasurementMode(uint32_t mode) {
    ...
    memcpy((char*)&p->data+sizeof(vpmm), &mode, sizeof(mode));
...
uint32_t Visualizer::initCaptureSize()
{
    ...
    uint32_t size = 0;
    if (status == NO_ERROR) {
        memcpy(&size, (char*)&p.data+sizeof(int32_t), sizeof(int32_t));
    }
    mCaptureSize = size;
